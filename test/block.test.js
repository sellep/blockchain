...
describe('Block', () =>
{
	...
	{
		expect(Block.difficulty(block, block.timestamp + 360000)).toBe(block.difficulty - 1);
	});

	it('increments difficulty when fast mined', () =>
	{
		expect(Block.difficulty(block, block.timestamp + 1)).toBe(block.difficulty + 1);
	});
});
